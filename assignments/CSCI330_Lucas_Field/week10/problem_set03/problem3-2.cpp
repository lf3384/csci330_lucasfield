...
class MediaFile {
 ...
 public:
  MediaFile(const std::string& name, size_t size)
    ...
    std::cout << "Loaded media file: " << filename << " (" << file_size
              << " bytes)" << std::endl;
  }
  ...
  void play() const { std::cout << "Playing: " << filename << std::endl; }
};
...
class Playlist {
 ...
 public:
  ...
  ~Playlist() { std::cout << "Destroyed playlist: " << name << std::endl; }

  void add_file(std::shared_ptr<MediaFile> file) {
    files.push_back(file);
    std::cout << "Added " << file->get_filename() << " to " << name
              << std::endl;
  }
...
class MediaLibrary {
 ...
 public:
  std::shared_ptr<MediaFile> load_media_file(const std::string& filename,
                                             size_t size) {
  ...
  void show_library_status() const {
    ...
    for (const auto& pair : media_files) {
      std::cout << "  " << pair.first << " (refs: " << pair.second.use_count()
                << ")" << std::endl;
    }
...
class NowPlayingTracker {
 ...
 public:
  void set_current_track(std::shared_ptr<MediaFile> track) {
    current_track = track;
    std::cout << "Now playing tracker set to: " << track->get_filename()
              << std::endl;
  }
  ...
  void show_current_track() const {
    ...
    if (track) {
      ...
      std::cout << "No track currently playing (or track was destroyed)"
                << std::endl;
    }
  }
};
